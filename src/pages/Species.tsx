import type { Species as SpeciesType } from "../types/swapi";
import { ResourceList } from "../components/ResourceList";
import { Link } from "react-router-dom";

export const Species = () => {
  return (
    <ResourceList<SpeciesType>
      resourceName="species"
      title="Species"
      renderItem={(spec) => {
        const id = spec.url.split("/").filter(Boolean).pop();
        return (
          <Link
            to={`/species/${id}`}
            key={spec.name}
          ...
          >
            <article className="h-full relative overflow-hidden rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900/40 p-5 transition-all duration-300 hover:border-zinc-300 hover:shadow-lg dark:hover:border-zinc-700 dark:hover:bg-zinc-900/80 flex flex-col justify-between">
              ...
              <div className="relative z-10">
                <h1 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-indigo-600 dark:group-hover:text-sky-400 transition-colors mb-4">
                  {spec.name}
                </h1>
